...
// Flag to enable or disable output classification
bool classify_output = false;

// Variables to track capture duration and total use-case execution time
#if ENABLE_TIME_PROFILING
int capture_time = 0, total_usecase_time = 0;
#endif
...
__NO_RETURN void AlgorithmThread (void *argument) {
  ...
  for (;;) {
    ...
    printf("Total usecase time: %3.3f ms.\n",
            profiler_cycles_to_ms(total_usecase_time, CPU_FREQ_HZ));
#endif
