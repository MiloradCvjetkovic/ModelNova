...
void StopMeasurements(int num_inferences) {
  ...
  ET_LOG(
      Info,
      "ethos-u : cycle_cnt : %lu cycles (%.2f per inference)",
      (unsigned long)ethosu_ArmBackendExecuteCycleCount,
  ...
  ET_LOG(
      Info,
      "Operator(s) total: %lu CPU cycles (%.2f per inference)",
      (unsigned long)ethosu_ArmBackendExecuteCycleCount,
      (double)ethosu_ArmBackendExecuteCycleCount / num_inferences);
  ...
  ET_LOG(
      Info,
      "cpu_wait_for_npu_cntr : %lu CPU cycles (%.2f per inference)",
      (unsigned long)ethosu_ArmWhenNPURunCycleCount,
      (double)ethosu_ArmWhenNPURunCycleCount / num_inferences);

  ET_LOG(Info, "Ethos-U PMU report:");
  ET_LOG(
      Info,
      "ethosu_pmu_cycle_cntr : %lu (%.2f per inference)",
      (unsigned long)ethosu_pmuCycleCount,
      (double)ethosu_pmuCycleCount / num_inferences);

  for (size_t i = 0; i < ethosu_pmuCountersUsed; i++) {
    ET_LOG(
        Info,
        "ethosu_pmu_cntr%d : %lu (%.2f per inference)",
        i,
        (unsigned long)ethosu_pmuEventCounts[i],
        (double)ethosu_pmuEventCounts[i] / num_inferences);
  }
#if defined(ETHOSU55) || defined(ETHOSU65)
  ET_LOG(
      Info,
      "Ethos-U PMU Events:[AXI0_RD_DATA_BEAT_RECEIVED, AXI1_RD_DATA_BEAT_RECEIVED, AXI0_WR_DATA_BEAT_WRITTEN, NPU_IDLE]");
#elif defined(ETHOSU85)
  ET_LOG(
      Info,
      "Ethos-U PMU Events:[SRAM_RD_DATA_BEAT_RECEIVED, SRAM_WR_DATA_BEAT_WRITTEN, EXT_RD_DATA_BEAT_RECEIVED, EXT_WR_DATA_BEAT_WRITTEN, NPU_IDLE]");
#else
#error No NPU target defined
#endif
}
